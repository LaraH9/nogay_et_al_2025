...
## Author(s): Yinxiu Zhan (modified by Lara Heckmann)
## Contact: yinxiu.zhan@fmi.ch (modifications: heckmann@ie-freiburg.mpg.de)
...
for file in $REF  $QUERY; do
        ...
        # Normalized BigWig
        bamCoverage  --scaleFactor $scaling_factor  --numberOfProcessors 5 --smoothLength 900 --binSize 300 --normalizeUsing None --centerReads -b $file  -o $name.bw 
 
        # Normalized BAM
        if (( $(echo "$scaling_factor < 1" | bc -l) )); then
            echo "Scaling BAM '$file' by $scaling_factor..."
            samtools view -s "$scaling_factor" -b "$file" > "${name}.seq_depth_norm.bam"
        else
            echo "Scaling factor ($scaling_factor) >= 1 for '$file' - skipping BAM normalization."
            cp "$file" "${name}.seq_depth_norm.bam"
    fi        

done


##outname=$name | sed -E 's/(.*_CUTTAG_[^_]+)_.*/\1/'
echo ""
echo "$name"
echo ""
mv stats_mapped_reads.txt $name.txt
mv out.sam $name.sam
